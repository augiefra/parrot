...
final class OutputMuteController {
    ...
    private func readVolumeControls(deviceID: AudioDeviceID) throws -> [VolumeControl] {
        if let control = try? readVolumeControl(
            deviceID: deviceID, api: .audioHardwareService, address: volumePropertyAddress()
        ) {
            return [control]
        }
        if let control = try? readVolumeControl(
    ...
    private func readVolumeControl(
        ...
        case .audioObject:
            ...
            status = AudioObjectGetPropertyData(deviceID, &mutableAddress, 0, nil, &size, &value)
        }
        guard status == noErr else { throw OutputMuteError.readVolume(status) }
        return VolumeControl(api: api, address: address, previousVolume: value)
    }
